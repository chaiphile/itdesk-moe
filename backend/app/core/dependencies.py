...
def require_org_scope(target_org_unit_id: int):
    ...
    def _dependency(current_user=Depends(get_current_user), db: Session = Depends(get_db), request: Request = None):
        ...
        if viewer_org_unit_id is None:
            ...
            try:
                write_audit(
                    ...
                    entity_id=target_org_unit_id if 'target_org_unit_id' in locals() else None,
                    meta={"path": getattr(request, "url", None) and getattr(request.url, "path", None), "method": getattr(request, "method", None)},
                    ip=(getattr(request, "client", None) and getattr(request.client, "host", None)),
