...
from app.models.models import User, Ticket, Role, Team, TeamMember, AuditLog
...
def test_agent_assign_scenarios(client, db):
    ...
    agent_c = User(username="agent_c", email="c@example.com", role_id=role_normal.id, org_unit_id=reg1.id, scope_level="REGION")
    agent_admin = User(username="agent_admin", email="adm@example.com", role_id=role_admin.id, org_unit_id=reg1.id, scope_level="REGION")
    agent_conf = User(username="agent_conf", email="p@example.com", role_id=role_conf.id, org_unit_id=reg1.id, scope_level="REGION")
    ...
    t2 = Ticket(title="T2", description="t2", created_by=agent_a.id, owner_org_unit_id=sch1.id, current_team_id=team_x.id, sensitivity_level="CONFIDENTIAL")
    # t3: out-of-scope (sch2), team_x
    t3 = Ticket(title="T3", description="t3", created_by=agent_a.id, owner_org_unit_id=sch2.id, current_team_id=team_x.id, sensitivity_level="REGULAR")
    db.add_all([t1, t2, t3])
    db.commit()
    db.refresh(t1)
    db.refresh(t2)
    ...
    # audit record
    audit = db.query(AuditLog).filter(AuditLog.entity_type == "ticket", AuditLog.entity_id == t1.id, AuditLog.action == "TICKET_ASSIGNED").first()
    assert audit is not None
    assert audit.diff_json["assignee_id"]["to"] == agent_a.id

    # 2) agent_a assign t1 to agent_b => non-admin should get 403
    resp = client.post(f"/agent/tickets/{t1.id}/assign", json={"assignee_id": agent_b.id}, headers=headers)
    assert resp.status_code == 403

    # 3) agent_a assign t1 to agent_c (different team) => 400
    resp = client.post(f"/agent/tickets/{t1.id}/assign", json={"assignee_id": agent_c.id}, headers=headers)
    ...
    assert resp.status_code == 403
    audit = db.query(AuditLog).filter(AuditLog.entity_type == "ticket_assign", AuditLog.entity_id == t3.id, AuditLog.action == "PERMISSION_DENIED").first()
    assert audit is not None
    ...
    audit = db.query(AuditLog).filter(AuditLog.entity_type == "ticket_confidential", AuditLog.entity_id == t2.id, AuditLog.action == "PERMISSION_DENIED").first()
    assert audit is not None

    # 6) privileged/admin agent can assign within team to others
    headers_admin = _auth_headers_for_user(agent_admin.username)
    resp = client.post(f"/agent/tickets/{t1.id}/assign", json={"assignee_id": agent_b.id}, headers=headers_admin)
    assert resp.status_code == 200
    data = resp.json()
    assert data["assignee_id"] == agent_b.id
    audit = db.query(AuditLog).filter(AuditLog.entity_type == "ticket", AuditLog.entity_id == t1.id, AuditLog.action == "TICKET_ASSIGNED").order_by(AuditLog.id.desc()).first()
    assert audit is not None
