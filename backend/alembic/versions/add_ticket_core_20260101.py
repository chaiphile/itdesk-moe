...
def upgrade() -> None:
    ...
    if "teams" in tables:
        ...
        if "org_unit_id" not in cols:
            op.add_column(
                "teams", sa.Column("org_unit_id", sa.Integer(), nullable=True)
            )
            op.create_foreign_key(
                "fk_teams_org_unit_id", "teams", "org_units", ["org_unit_id"], ["id"]
            )
            op.create_index(
                op.f("ix_teams_org_unit_id"), "teams", ["org_unit_id"], unique=False
            )
    ...
    if "team_members" not in tables:
        ...
        op.create_index(
            op.f("ux_team_members_team_user"),
            "team_members",
            ["team_id", "user_id"],
            unique=True,
        )
    ...
    if "tickets" in tables:
        ...
        if "owner_org_unit_id" not in cols:
            op.add_column(
                "tickets", sa.Column("owner_org_unit_id", sa.Integer(), nullable=False)
            )
            op.create_foreign_key(
                "fk_tickets_owner_org_unit_id",
                "tickets",
                "org_units",
                ["owner_org_unit_id"],
                ["id"],
            ...
            op.create_index(
                "idx_tickets_owner_org_unit_id", "tickets", ["owner_org_unit_id"]
            )
        if "assignee_id" not in cols:
            op.add_column(
                "tickets", sa.Column("assignee_id", sa.Integer(), nullable=True)
            )
            op.create_foreign_key(
                "fk_tickets_assignee_id", "tickets", "users", ["assignee_id"], ["id"]
            )
        if "category_id" not in cols:
            op.add_column(
                "tickets", sa.Column("category_id", sa.Integer(), nullable=True)
            )
            op.create_foreign_key(
...
def downgrade() -> None:
    ...
    if "ticket_messages" in tables:
        op.drop_index(
            "idx_ticket_messages_ticket_id_created_at", table_name="ticket_messages"
        )
    ...
    if "tickets" in tables:
        ...
        if "owner_org_unit_id" in cols:
            try:
                ...
                pass
            op.drop_constraint(
                "fk_tickets_owner_org_unit_id", "tickets", type_="foreignkey"
            )
