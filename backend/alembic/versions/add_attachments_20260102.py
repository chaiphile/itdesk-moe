...
def upgrade() -> None:
    ...
    if 'attachments' not in tables:
        op.create_table(
            ...
            sa.Column('scanned_status', sa.String(), nullable=False, server_default='PENDING'),
        ...
        op.create_index('idx_attachments_scanned_status', 'attachments', ['scanned_status'])
        op.create_index('ux_attachments_object_key', 'attachments', ['object_key'], unique=True)
        op.create_check_constraint('ck_attachments_scanned_status_vals', 'attachments', "scanned_status IN ('PENDING','CLEAN','INFECTED','FAILED')")
...
def downgrade() -> None:
    ...
    if 'attachments' in tables:
        ...
        try:
            ...
            pass
        try:
            op.drop_constraint('ck_attachments_scanned_status_vals', 'attachments', type_='check')
        except Exception:
            pass
